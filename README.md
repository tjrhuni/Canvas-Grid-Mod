...
- 도화지 위에 32×32 모눈을 그립니다. 4칸마다 굵은 선을 넣고 위쪽과 왼쪽에 눈금 숫자(4, 8, 12, …
  28)를 붙여, 도안의 좌표를 보면서 칸을 세지 않고도 위치를 찾을 수 있습니다.
- 찍은 픽셀은 클라이언트가 이미 받은 지도 데이터로 또렷한 정사각형으로 다시 그려집니다. 가까이서
  봐도 픽셀이 선 밑으로 번지지 않고 모눈 칸 안에 정확히 들어갑니다.
- 선 굵기, 불투명도, 색상, 굵은 선 간격은 설정 화면에서 바꿀 수 있습니다.

### 2. 포인터: 실제로 찍히는 픽셀 표시

- ArtMap은 십자선이 닿는 자리가 아니라 앉은 사람의 시선 각도로 픽셀을 정합니다. 그래서 도화지
  가장자리로 갈수록 십자선이 가리키는 자리와 실제로 찍히는 픽셀이 어긋났습니다.
- 이 모드는 ArtMap과 같은 각도표로 계산해, 이젤에 앉아 있을 때 지금 클릭하면 칠해질 픽셀에
  테두리를 그립니다. 테두리가 서버와 일치하는 쪽이므로 십자선 대신 테두리를 보고 찍으면 됩니다.
- 테두리의 색과 굵기는 설정에서 바꿀 수 있습니다.

### 3. 픽셀 좌표 표시

- 포인터가 가리키는 픽셀의 좌표를 "픽셀 16, 14"처럼 화면 모서리에 표시합니다. 왼쪽 위 픽셀이
  1, 1입니다.
- 표시 위치는 오른쪽 아래 등 네 모서리 중 하나로 고를 수 있습니다.

### 그 밖에

- 게임 안 설정 화면이 있습니다(Mod Menu 또는 단축키). 항목마다 설명 툴팁이 있고 한국어와 영어를
  지원합니다.
